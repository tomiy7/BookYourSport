...
public class RefundCreditHandlerTests
{
    ...
    public async Task Handle_ShouldRefund_WhenRefundIsAllowed()
    {
        ...
        var repository = new FakeCreditAccountRepository { Account = account };
        ...
        var reservationStart = new DateTime(2026, 8, 20, 18, 0, 0);
        var cancellationTime = new DateTime(2026, 8, 19, 18, 0, 0);
        ...
        Assert.Equal(40, refundTransaction.Amount);
        Assert.Equal(reservationId, refundTransaction.ReferenceId);
    }
    ...
    public async Task Handle_ShouldNotRefund_WhenRefundIsNotAllowed()
    {
        ...
        var reservationStart = new DateTime(2026, 8, 20, 18, 0, 0);
        var cancellationTime = new DateTime(2026, 8, 20, 7, 0, 0);
    ...
    {
        var repository = new FakeCreditAccountRepository { Account = null };
        var refundPolicy = new RefundPolicy();
        ...
        Assert.Equal("Credit account not found.", exception.Message);
        Assert.Null(repository.SavedAccount);
    }
    ...
    public async Task Handle_ShouldApplyPartialRefund_WhenCancellationIsBetween12And24Hours()
    {
        ...
        var account = new CreditAccount(userId);
        account.TopUp(100, Guid.NewGuid());
        account.Charge(100, reservationId);
        ...
        var reservationStart = new DateTime(2026, 8, 20, 18, 0, 0);
        var cancellationTime = new DateTime(2026, 8, 20, 2, 0, 0);
        ...
        var refundTransaction = account.Transactions.Last();
        Assert.Equal(TransactionType.Refund, refundTransaction.Type);
        Assert.Equal(50, refundTransaction.Amount);
        Assert.Equal(reservationId, refundTransaction.ReferenceId);
    }
    ...
    public async Task Handle_ShouldThrow_WhenRefundAlreadyProcessed()
    {
        ...
        Assert.Equal("Refund has already been processed.", exception.Message);
        Assert.Equal(100, account.Balance);
    }
}